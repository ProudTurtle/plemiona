...
const $table = $(data).find(".table-responsive table").first();
const $ths = $table.find("tr:first th");
// szukamy th z ikonką miecza (nadchodzące ataki)
const attackTh = $ths.filter((_, th) => {
return $(th).find('img[src*="unit/att"]').length > 0;
}).first();
if (attackTh.length) {
const idx = attackTh.index();
const $playerRow = $table.find('tr').eq(1); // pierwszy wiersz z graczem
attackCount = $playerRow.children().eq(idx).text().trim() || "0";
} else {
...
let playerVillages = [];
const hasIncomings = $(data).find('.table-responsive table tr:first th img[src*="unit/att"]').length > 0;
...
$("#" + allCategories + "Button").attr("class", "btn evt-confirm-btn btn-confirm-no");
}
}
}
...
stacklistBB += `[*]Taka wioska nie istnieje[|]${element[0].playerName}[|]${element[0].tribeName}[|]${numberWithCommas(element[0].totalStack)}[|]${Math.round((targetStackSize - element[0].totalStack) / packetSize)}\n`;
}
else {
...
stacklistBB += `[*]Taka wioska nie istnieje[|]${numberWithCommas(element[0].totalStack)}[|]${Math.round((targetStackSize - element[0].totalStack) / packetSize)}\n`;
}
else {
selectedVillages.splice(-1, 1);
$(`[id="map_village_${village.id}"]`).css({
